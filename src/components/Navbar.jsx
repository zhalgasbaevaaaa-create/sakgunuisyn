...
import { Compass, BookOpen, Map, Users, Clock, ShieldAlert, Award, Search, Menu, X, CheckCircle } from 'lucide-react';
...
export default function Navbar({ activeSection, setActiveSection, openSearch, progress, score, totalQuestions }) {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled 
        ? 'bg-obsidian-950/90 backdrop-blur-md border-b border-gold-500/20 py-2.5 shadow-xl' 
        : 'bg-gradient-to-b from-obsidian-950/90 to-transparent py-4'
    }`}>
      {/* Top Reading Progress Bar */}
      <div className="absolute top-0 left-0 right-0 h-1 bg-obsidian-800">
        <div 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        {/* Desktop Nav Items */}
        <nav className="hidden lg:flex items-center space-x-1 bg-obsidian-800/60 p-1.5 rounded-full border border-gold-500/10 backdrop-blur-sm">
          {navItems.map((item) => {
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-lg bg-obsidian-800 border border-gold-500/20 text-gold-400 hover:text-gold-300"
          >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-obsidian-950/95 border-b border-gold-500/20 px-4 py-4 mt-2 backdrop-blur-xl animate-fadeIn">
